...
public class Maze {
    ...
    public Point[] getChildren(Point point) {
        // point node has x, y, and parent node
        if (point == null) {
            return null;
        }
        ...
        if (!this.north[x][y]) {
            if (!(x == xP && y+1 == yP)) {
                children[count] = getResult(x, y + 1, point);
                count++;
            }
        }
        if (!this.south[x][y]) {
        ...
        if (!this.east[x][y]) {
            if (!(x + 1 == xP && y == yP)) {
                children[count] = getResult(x + 1, y, point);
                count++;
            }
        }
        if (!this.west[x][y]) {
    ...
    private Point getResult(int x, int y, Point parent) {
    ...
    public void solveDFSIterative() {
        ...
        while (!current.equals(this.end)) {
            for (Point p : getChildren(current)) {
                pointsStack.push(p);
            }
